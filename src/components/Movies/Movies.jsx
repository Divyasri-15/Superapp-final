...
function Movies({genre}){
    ...
    return(
        <>
            <p style={{ color:"white",fontSize:"1.7rem",marginLeft:"5vw",color:"#878787",marginBottom:"1%"}}>{genre}</p>
            <div style={{display:"flex",marginLeft:"5vw",justifyContent:"space-between",width:"90vw"}}>
                {movies.map((movie,idx)=>{
                    return (
                    <div key={idx} style={{width:"20vw"}}>
                        <img src={movie?.primaryImage?.url} style={{objectFit:"cover",width:"20vw",marginBottom:"1vw",height:"15vh",borderRadius:"12px"}}/>
                    </div>
                    )
                })}
            </div>
